...
int main (int argc, char ** argv) {
  ...
  while ( mixer.next_event() && iev < nEvent )
  {
    ...
    for(PseudoJet jet : jetCollectionSig.getJet()) {
      ...
      */
      ...
      //trw.addCollection("label", label);
      //trw.addCollection("jet_energy", jet_energy);
      //trw.addCollection("jet_pt", jet_pt);
      trw.fillTree();
    } //end jet loop
  ...
  TFile *fout = new TFile(cmdline.value<string>("-output", "Data_Test_groot.root").c_str(), "RECREATE");
  trOut->Write();
  fout->Write();
